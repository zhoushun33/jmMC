...
document.addEventListener("DOMContentLoaded",function(){
	// 获取DOM元素对象
	//收样照片
	hs_jczp=document.getElementById("history_jczp");
	ep_jczp=document.getElementById("empty_jczp");		
...
function createItem( p,entry,lx ) {
	...
	if (lx=='jczp'||lx=='jczp2') {
		hs=hs_jczp;
		ep=ep_jczp;
		files_jczp.push({name:"upfile"+index_jczp,path:p});
		index_jczp++;
	}
	// 设置空项不可见
...
function cleanHistory(lx) {	
	...
	mui.confirm('您确定要清空记录吗？', '警告:', btnArray, function(e) {
		if (e.index == 0) {
			if (lx=='jczp'||lx=='jczp2') {				
				hs_jczp.innerHTML = '<li id="empty_jczp" class="ditem-empty">无历史记录</li>';
				ep_jczp=document.getElementById("empty_jczp");		
				files_jczp=[];
				index_jczp=1;
			}
...
//			outLine("上传成功："+t.responseText);
	...
	task.addData("lx",lx);
	task.addData("zt",zt);
	task.addData("test_result",get_result());
	task.addData("testNum",getNum());
